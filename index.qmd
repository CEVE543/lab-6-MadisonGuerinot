...
fname = "example.nc"
if !isfile(fname); 
  download("https://archive.unidata.ucar.edu/software/netcdf/examples/tos_O1_2001-2002.nc", "example.nc")
end
...
```{julia}
#lots of different ways to call the data
tos[lon = 1, lat = 1]
...
local_nc_path = "tas_region_100E_88N.nc" # We will save our slice here as NetCDF but could use a different back end
```
...
if !isfile(local_nc_path)
    ...
    # Use a 'do' block to safely open and close the remote dataset
    open_dataset(zopen(remote_store_url, consolidated=true)) do g
        # Get the 'tas' variable (still lazy)
        tas = g["tas"]
        
        # Define the regional slice (still lazy)
...
# 3. Now, perform the analysis on the *local file*
# We use another 'do' block to open/close the local NetCDF safely
println("Calculating mean from local file...")
avg_tas = open_dataset(local_nc_path) do region
    # This read is now from local disk, so it's very fast
    mean(skipmissing(region))
end
...
```{julia}
#= 
#pull the tas data, defined as c
tas = g["tas"];
=#
```
...
#define rectangle
using DimensionalData
region = tas[lon = 100 .. 101, lat = 88.. 88.5]
=#
```
...
#= 
using Statistics
avg_tas = mean(skipmissing(region))
=#
```
```{julia}
#= 
...
plot the time series of `tas` for that region.
```{julia}
#= 
avg_tas_time = mean(region; dims=(:lat, :lon))
time = region.time  # get the time coordinate
=#
```
```{julia}
#= 
...
ax = Axis(fig[1, 1], xlabel = "Time", ylabel = "tas (K)",
          title = "Spatially Averaged tas Time Series")

lines!(ax, time, avg_tas_time, color = :blue, linewidth = 2)
...
#= 
using StatsBase
idx_hottest = argmax(avg_tas_time_array)
idx_coldest = argmin(avg_tas_time_array)

println("Hottest period at time: ", time_array[idx_hottest])
println("Coldest period at time: ", time_array[idx_coldest])
=#
```

```{julia}
#= 
#get the point in time across the globe
tas_hottest = tas[idx_hottest, :, :] 
tas_coldest = tas[idx_coldest, :, :]
=#
```

```{julia}
#= 
fig, ax, plt = heatmap(tas_hottest; colormap = :seaborn_icefire_gradient,
    axis = (; aspect=DataAspect()),
    figure = (; size = (1200,600), fontsize=24))
fig
=#
...
```{julia}
#= 
fig, ax, plt = heatmap(tas_coldest; colormap = :seaborn_icefire_gradient,
    axis = (; aspect=DataAspect()),
    figure = (; size = (1200,600), fontsize=24))
fig
=#
